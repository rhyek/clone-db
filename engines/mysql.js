...
module.exports = {
    ...
    return [
      {
        ...
        command: `${ config.source.ssh ? `ssh -p ${ config.source.port || 22 } ${ config.source.host } ` : '' }${ config.source.db.password ? `MYSQL_PWD="${ config.source.db.password }"` : '' } mysqldump -u ${ config.source.db.username } -R ${ config.source.db.database} | sed -e 's/DEFINER[ ]*=[ ]*[^*]*\\*/\\*/' | sed -e 's/DEFINER[ ]*=[ ]*[^*]*PROCEDURE/PROCEDURE/' | sed -e 's/DEFINER[ ]*=[ ]*[^*]*FUNCTION/FUNCTION/' | gzip -c9 > /tmp/clonedbdump.gz`
      },
      {
        message: 'Decompress.',
  ...
  runSQL (config, sql) {
    return `${ config.target.ssh ? `ssh -p ${ config.target.port || 22 } ${ config.target.host } ` : '' }${ config.source.db.password ? `MYSQL_PWD="${ config.target.db.password }"` : '' } mysql -u ${ config.target.db.username } -e "${ sql }" ${ config.target.db.database }`
  }
}