...
module.exports = {
  ...
  runSQL (config, sql) {
    return `${ config.target.ssh ? `ssh -p ${ config.target.port || 22 } ${ config.target.host } ` : '' }${ config.target.db.password ? `PGPASSWORD="${ config.target.db.password }"` : '' } psql -h localhost -U ${ config.target.db.username } -c "${ sql }" ${ config.target.db.database }`
  }
}